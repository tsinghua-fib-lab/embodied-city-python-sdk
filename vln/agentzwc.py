...
class AirsimAgent:
    def __init__(self, cfg, query_func, prompt_template, uav_name='UAV0'):
        self.query_func = query_func
    ...
    def init_config(self):
        ...
        self.client.enableApiControl(True, self.uav_name)
        self.client.armDisarm(True, self.uav_name)
        self.client.takeoffAsync(vehicle_name=self.uav_name)
    ...
    def setVehiclePose(self, x, y, z, pitch, roll, yaw) -> None:
        '''
        pose为[pos, rot]
        rot接受欧拉角或者四元数，
        ...
        pose = np.array([x, y, z, pitch, roll, yaw])
        ...
        self.client.simSetVehiclePose(air_pose, True, vehicle_name=self.uav_name)
    ...
    def moveToPosition(self, position):
        ...
        self.client.moveToPositionAsync(float(pos_world[0]), float(pos_world[1]), float(pos_world[2]),
                                        self.velocity, vehicle_name=self.uav_name).join()
    ...
    def turnLeft(self, yaw=np.pi / 4):
        ...
        self.client.moveByRollPitchYawZAsync(0, 0, float(new_yaw_body), float(cur_pos[2]), 2,
                                             vehicle_name=self.uav_name).join()
    ...
    def get_panorama_images(self, image_type=0):
        ...
        for angle in self.panoid_yaws:
            ...
            self.client.moveByRollPitchYawZAsync(0, 0, float(yaw), float(cur_pos[2]), 2,
                                                 vehicle_name=self.uav_name).join()
            image = self.get_front_image(image_type)
            panorama_images.append(image)

        self.client.moveByRollPitchYawZAsync(0, 0, float(cur_yaw_body), float(cur_pos[2]), 2,
                                             vehicle_name=self.uav_name).join()
    ...
    def get_front_image(self, image_type=0):
        ...
        responses = self.client.simGetImages(
            ...
            vehicle_name=self.uav_name)  # if image_type == 0:
    ...
    def get_xyg_image(self, image_type, cameraID):
        ...
        if image_type == 0:
            responses = self.client.simGetImages(
                [airsim.ImageRequest(cameraID, airsim.ImageType.Scene, False, False)],
                vehicle_name=self.uav_name)  # if image_type == 0:
            response = responses[0]
            ...
            # 获取DepthVis深度可视图
            responses = self.client.simGetImages([
                airsim.ImageRequest(cameraID, airsim.ImageType.DepthPlanar, True, False)], vehicle_name=self.uav_name)
    ...
    def get_current_state(self):
        ...
        state = self.client.simGetGroundTruthKinematics(vehicle_name=self.uav_name)
    ...
    def get_image_by_yaw(self, yaw=0, image_type=0):
        ...
        self.client.moveByRollPitchYawZAsync(0, 0, float(target_yaw), float(cur_pos[2]), 1,
                                             vehicle_name=self.uav_name).join()
        time.sleep(1)
        # self.client.rotateToYawAsync(-yaw).join()
        image = self.get_front_image(image_type)
        self.client.moveByRollPitchYawZAsync(0, 0, float(cur_yaw_body), float(cur_pos[2]), 1,
                                             vehicle_name=self.uav_name).join()
        # self.client.rotateToYawAsync(yaw).join()
        return image
    ...
    def fly_to_p(self, goal, obstruct_dis=10, step_dis=10, target_dis=10, yaw_bias=30):
        ...
        self.client.rotateToYawAsync(yaw, vehicle_name=self.uav_name).join()
        # 水平移动
    ...
    def move_to_ground_above(self):
        ...
        while np.equal(bird_eye_view[int(width / 2 - t * width):int(width / 2 + t * width),
                       int(height / 2 - t * height):int(height / 2 + t * height)], [28, 67, 121]).any():
