...
class AirsimAgent:
    def __init__(self, cfg, query_func, prompt_template):
        ...
        self.velocity = 4
        self.panoid_yaws = [0,np.pi/2,np.pi,3*np.pi/2]
    ...
    # def body2global_rotation(self, global_rot, body_rot):
    ...
    def worldframe2bodyframe(self, worldframe):
        if type(worldframe) is not np.ndarray:
            worldframe = np.array(worldframe)

        cur_pos, cur_rot = self.get_current_state()
        cur_rot = R.from_euler("XYZ", cur_rot).as_matrix()
        global2body_rot = self.global2body_rotation(cur_rot, self.rotation)
        bodyframe = np.linalg.inv(global2body_rot).dot(worldframe - cur_pos)
        return bodyframe

    # position is in current body frame
    def moveToPosition(self, position):
        pos_world = self.bodyframe2worldframe(position)
        # print("move to(worldframe):",pos_world)
...
#################################
    ...
    def get_image_by_yaw(self,yaw=0,image_type=0):
        ...
        self.client.moveByRollPitchYawZAsync(0, 0, float(target_yaw), float(cur_pos[2]), 2).join()
        image = self.get_front_image(image_type)
        self.client.moveByRollPitchYawZAsync(0, 0, float(cur_yaw_body), float(cur_pos[2]), 2).join()
        return image
